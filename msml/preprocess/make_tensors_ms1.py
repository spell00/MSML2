...
if 'logs' not in os.listdir():
    os.makedirs('logs')
logging.basicConfig(filename='logs/make_tensors_ms1_split.log', filemode='w',
...
def get_data_matrix(args):
    ...
    dir_name = f"{script_dir}/{out_dest}/mz{args.mz_bin}/rt{args.rt_bin}/mzp{args.mz_bin_post}/" \
               f"rtp{args.rt_bin_post}/{args.spd}spd/ms1/combat{args.combat_corr}/shift{args.shift}/" \
               f"{args.scaler}/log{args.log2}/{args.feature_selection}/"
    dir_input = f"{script_dir}/{input_dir}/mz{args.mz_bin}/rt{args.rt_bin}/{args.spd}spd/ms1/all/"
...
if __name__ == "__main__":
    ...
    data_matrix, not_zeros_col = pd.concat([x[0] for x in data_matrix], axis=1), np.array(
