...
def find_not_zeros(data_matrix):
    ...
    if args.n_cpus < 1:
        ...
        n_cpus = args.n_cpus
    with multiprocessing.Pool(n_cpus) as pool:
        fun = MultiKeepNotFunctions(keep_not_zeros, dframe_list, threshold=0.9,
...
if __name__ == "__main__":
    ...
    dir_name = f"{script_dir}/{out_dest}/mz{args.mz_bin}/rt{args.rt_bin}/mzp{args.mz_bin_post}/" \
               f"rtp{args.rt_bin_post}/{args.spd}spd/ms2/" \
    ...
    train_data = pd.DataFrame(data_matrix.values, index=labels, columns=columns, dtype=np.float32)
    # labels = data_matrix.index
    lows = []
    cats = []
    batches = []
