...
class LoadTensorsMultiprocess:
    ...
    def process(self, i):
        ...
        final = [self.load_csv(label, j) for j in
             os.listdir(f"{self.path}/csv3d/{label}")]
        return np.stack(final), [x.split('_')[-1].split('.csv')[0] for x in os.listdir(f"{self.path}/csv3d/{label}")], label
    ...
    def load_csv(self, label, i):
        # final.to_csv(f"{self.path}/csv3d/{label}/{label}_{i}.csv", index_label='ID')
        df = pd.read_csv(f"{self.path}/csv3d/{label}/{i}", index_col='ID')
        return df
...
def make_df(dirinput, dirname, bins, args_dict, names_to_keep=None, features=None):
    ...
    concat = LoadTensorsMultiprocess(lists["tsv"], lists["labels"], bins,
    ...
    data_matrix = pool.map(concat.process,
                           range(len(concat.labels_list))
                           )
    print('Tensors are loaded.')
    mz_min_parents = [float(x) for x in data_matrix[0][1]]
    labels = [x[2] for x in data_matrix]
    # Find max of each dimension
    matrix = [np.stack(x[0]) for x in data_matrix]
    # matrix = []
    # for i, x in enumerate(data_matrix):
    #     # np.stack(x[0])
    #     try:
    #         matrix.append(np.stack(x[0]))
    #     except:
    #         for y in x[0]:
    #             print(y.shape)
    ...
    if features is not None:
        ...
        max_rt2 = max([x.shape[1] for x in matrix])
        max_mz2 = max([x.shape[2] for x in matrix])
        max_rt = int(max(max_rt1, max_rt2))
        max_mz = int(max(max_mz1, max_mz2))
    else:
        max_rt = max([x.shape[1] for x in matrix])
        max_mz = max([x.shape[2] for x in matrix])
    ...
    matrices = []
    new_labels = []
    for i, (matrix, label) in enumerate(zip([x[0] for x in data_matrix], labels)):
        if matrix.shape[0] < max_min_mz_parent:
            matrix = np.concatenate(
                (matrix, np.zeros((max_min_mz_parent-matrix.shape[0], matrix.shape[1], matrix.shape[2]))), 0)
