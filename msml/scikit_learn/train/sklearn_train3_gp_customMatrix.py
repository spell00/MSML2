...
if __name__ == "__main__":
    ...
    if args.combat_data and args.combat:
        args.train_input = f"{args.input_dir}/mz{args.mz}/rt{args.rt}/{args.spd}spd/combat{args.combat}/" \
                           f"stride{args.stride}/{args.preprocess_scaler}/log{args.log2}/train/{args.run_name}/" \
                           f"BACT_train_inputs_gt0.0_{args.run_name}_combat.csv"
    else:
        args.train_input = f"{args.input_dir}/mz{args.mz}/rt{args.rt}/{args.spd}spd/combat{args.combat}/" \
                           f"stride{args.stride}/{args.preprocess_scaler}/log{args.log2}/train/{args.run_name}/" \
                           f"BACT_train_inputs_{args.run_name}.csv"
    args.valid_input = f"{args.input_dir}/mz{args.mz}/rt{args.rt}/{args.spd}spd/combat{args.combat}/" \
                       f"stride{args.stride}/{args.preprocess_scaler}/log{args.log2}/train/{args.run_name}/" \
                       f"BACT_valid_inputs_{args.run_name}.csv"

    if args.inference_inputs and args.combat_data and args.combat:
        args.inference_inputs = f"{args.input_dir}/mz{args.mz}/rt{args.rt}/{args.spd}spd/" \
                                f"combat{args.combat}/stride{args.stride}/{args.preprocess_scaler}" \
                                f"/log{args.log2}/valid/{args.run_name}/BACT_inference_inputs_gt0.0_{args.run_name}_combat.csv"
    elif args.inference_inputs:
        args.inference_inputs = f"{args.input_dir}/mz{args.mz}/rt{args.rt}/{args.spd}spd/" \
                                f"combat{args.combat}/stride{args.stride}/{args.preprocess_scaler}" \
                                f"/log{args.log2}/valid/{args.run_name}/BACT_inference_inputs_{args.run_name}.csv"
    ...
    if args.drop_lows == "train":
        train_data, train_labels, train_batches, train_lows, train_highs, train_vhighs, train_blks = new_load_data(
            args.train_input, binary=args.binary,
            min_mz=args.min_mz, min_rt=args.min_rt)
        # TODO CHANGE THIS, THIS IS JUST SOME QUICK FIX FOR FLORENCE's POSTER
        valid_data, valid_labels, valid_batches, valid_lows, valid_highs, valid_vhighs, valid_blks = new_load_data(
            args.valid_input, binary=args.binary,
            min_mz=args.min_mz, min_rt=args.min_rt)

        train_data = pd.concat([train_data, valid_data])
        train_labels = np.concatenate([train_labels, valid_labels])
        train_batches = np.concatenate([train_batches, valid_batches])
        train_lows = np.concatenate([train_lows, valid_lows])
        train_highs = np.concatenate([train_highs, valid_highs])
        train_vhighs = np.concatenate([train_vhighs, valid_vhighs])
        train_blks = np.concatenate([train_blks, valid_blks])
        if args.inference_inputs != 0:
            test_data, test_labels, test_batches, test_lows, test_highs, test_vhighs, test_blks = new_load_data(
                args.inference_inputs, binary=args.binary,
                min_mz=args.min_mz, min_rt=args.min_rt, valid_batch=1)

        else:
            skf = StratifiedKFold(n_splits=5)
            train_nums = np.arange(0, len(train_labels))
            train_inds, test_inds = skf.split(train_nums, ["_".join([x, str(y)]) for x, y in
                                                           zip(train_labels, train_lows)]).__next__()
        ...
        if args.threshold != 0:
            good_features = np.array(
                [i for i in range(train_data.shape[1]) if
                 sum(train_data[:, i] != 0) > int(train_data.shape[0] * args.threshold)])
