...
class Train:
    def __init__(self, path, model, model_name='resnet18', get_data_function=MSCSV, n_channels=1, verbose=1, cv=5,
                 ...
                 epochs_per_print=1, test=False, resize=False, train_high_only=False,
                 remove_padding=False, scaler='binarize', domain_penalty=True):

        process = get_data_function(path, scaler, resize, test)
    ...
    def train(self, params):
        ...
        if self.load:
            cnn, optimizer, epoch, losses, kl_divs, losses_recon, best_loss = load_checkpoint(checkpoint_path=log_path,
                                                                                              model=cnn,
                                                                                              optimizer=optimizer,
                                                                                              name=f"vaecnn.pth")
...
if __name__ == "__main__":
    ...
    best_parameters, values, experiment, model = optimize(
        parameters=[
            ...
            {"name": "C", "type": "range", "bounds": [1e-5, 1e-3], "log_scale": True},
