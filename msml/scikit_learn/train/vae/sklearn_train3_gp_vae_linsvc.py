...
class Train:
    ...
    def train(self, params):
        ...
        if self.load:
            cnn, optimizer, epoch, losses, kl_divs, losses_recon, best_loss = load_checkpoint(checkpoint_path='logs/vaecnn/max/b0.001/dop0/aeoptmse/bin0/h0/rempad0/optadam/d0.019/trans_0_0/shear0/blur_0_0.001/cscal0/cratio0/escal0/eratio0/lr7.618e-05/wd3.1e-07/l10.0046845',
                                                                                              model=cnn,
                                                                                              optimizer=optimizer,
                                                                                              name=f"vaecnn.pth")
        ...
        for i, (train_inds, valid_inds) in enumerate(skf.split(all_data, ['_'.join([str(cat), str(low)]) for cat, low in zip(all_cats, all_lows)])):
            ...
            for _ in range(1):
                ...
                for i, batch in enumerate(train_loader):
                    ...
                    enc, rec, losses, kld = cnn(data, sampling=False)
