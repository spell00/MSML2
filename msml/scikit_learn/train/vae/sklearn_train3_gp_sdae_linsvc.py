...
class Train:
    ...
    def train(self, params):
        ...
        if self.load:
            cnn, optimizer, epoch, losses, kl_divs, losses_recon, best_loss = load_checkpoint(checkpoint_path='logs/sdaecnn/max/dop0/aeoptmse/bin0/h0/rempad0/optadam/d0.084/trans_0_0/shear0/blur_0_0/cscal0/cratio0/escal0/eratio0/lr5.859e-05/wd3.1e-07/l10.0046845',
                                                                                              model=cnn,
                                                                                              optimizer=optimizer,
                                                                                              name=f"sdaecnn.pth")
        ...
        for i, (train_inds, valid_inds) in enumerate(skf.split(all_data, ['_'.join([str(cat), str(low)]) for cat, low in zip(all_cats, all_lows)])):
            ...
            best_loss = -1
            train_set = MSDataset(train_data, False, train_labels, train_cats, train_lows, train_batches,
                                  transform=transform_train, crop_size=-1,
                                  quantize=False, remove_paddings=self.remove_padding, device='cuda')
            valid_set = MSDataset(valid_data, False, valid_labels, valid_cats, valid_lows, valid_batches,
                                  transform=transform_valid, crop_size=-1,
                                  quantize=False, device='cuda')
            test_set = MSDataset(test_data, False, test_labels, test_cats, test_lows, test_batches,
                                 transform=transform_valid, crop_size=-1,
                                 quantize=False, device='cuda')
