...
class Train:
    def __init__(self, model, data, hparams_names, args, ovr, binary=True):
        ...
        try:
            with open(f'{args.output_dir}/saved_models/sklearn/best_params.json', "r") as json_file:
    ...
    def train(self, h_params):
        ...
        if args.scaler != 'binarize':
            ...
            scaler = Pipeline([('minmax', get_scaler('minmax')()), ('binarizer', Binarizer(threshold=args.thres))])
        ...
        if score > np.mean(self.best_scores_valid) and score > float(self.previous_models[model]['valid_acc_mean']):
            ...
            save_confusion_matrix(fig, f"{args.output_dir}/confusion_matrices/{model}_valid", acc=score)
            fig = get_confusion_matrix(y_valids_highs, y_pred_valids_highs, unique_labels)
            save_confusion_matrix(fig, f"{args.output_dir}/confusion_matrices/{model}_valid_highs", acc=np.mean(self.best_scores_valid_highs))
            fig = get_confusion_matrix(y_valids_lows, y_pred_valids_lows, unique_labels)
            save_confusion_matrix(fig, f"{args.output_dir}/confusion_matrices/{model}_valid_lows", acc=np.mean(self.best_scores_valid_lows))
            try:
                self.best_roc_score = save_roc_curve(m, x_valids, y_valids, unique_labels,
                                                     f"{args.output_dir}/ROC/{model}_valid", binary=args.binary,
                                                     acc=score)
            except:
                pass
            try:
                self.best_roc_score_highs = save_roc_curve(m, x_valids_highs, y_valids_highs, unique_labels,
                                                     f"{args.output_dir}/ROC/{model}_valid_highs", binary=args.binary,
            ...
            try:
                self.best_roc_score_lows = save_roc_curve(m, x_valids_lows, y_valids_lows, unique_labels,
                                                     f"{args.output_dir}/ROC/{model}_valid_lows", binary=args.binary,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, help="Path to results")
    parser.add_argument("--fname", type=str, help="name csv file")
    ...
    parser.add_argument("--combat_data", type=int, default=1)
    parser.add_argument("--stride", type=str, default=0)

    args = parser.parse_args()

    args.train_input = f"{args.input_dir}/{args.fname}"
    ...
    train_data, train_labels, train_batches, train_lows, train_highs, train_vhighs, train_blks = new_load_data(args.train_input, binary=args.binary,
                                                                        min_mz=args.min_mz, min_rt=args.min_rt)
    if args.inference_inputs != 0:
        ...
        train_inds, test_inds = skf.split(train_nums, ["_".join([x, str(y)]) for x, y in zip(train_labels, train_lows)]).__next__()
        train_data, test_data = train_data.values[train_inds], train_data.values[test_inds]
        ...
        train_lows, test_lows = train_lows[train_inds], train_lows[test_inds]
        train_highs, test_highs = train_highs[train_inds], train_highs[test_inds]
        train_blks, test_blks = train_blks[train_inds], train_blks[test_inds]
        train_vhighs, test_vhighs = train_vhighs[train_inds], train_vhighs[test_inds]

    # train_data = train_data.values
    if args.threshold == 0:
        good_features = np.array(
            [i for i in range(train_data.shape[1]) if sum(train_data[:, i] != 0) > int(train_data.shape[0] * args.threshold)])
    ...
    os.makedirs(f"{args.output_dir}/saved_models/sklearn/", exist_ok=True)
    for model in models:
        ...
        if args.correct_batches:
            ...
            try:
                high_test_data = data["highs"]["data"][:, :features_cutoff]
            except:
                high_test_data = data["highs"]["data"].iloc[:, :features_cutoff]

            try:
                vhigh_test_data = data["vhighs"]["data"][:, :features_cutoff]
            except:
                vhigh_test_data = None
        ...
        if low_test_data is not None:
            ...
            if float(best_params_dict[model]['valid_acc_mean']) >= float(previous_models[model]['valid_acc_mean']):
                ...
                save_confusion_matrix(fig, f"{args.output_dir}/confusion_matrices/{model}_lows_test",
                                      acc=low_test_score)
                try:
                    best_params_dict[model]['test_lows_auc'] = save_roc_curve(m, low_test_data, low_test_classes,
                                                                         unique_labels,
                                                                         f"{args.output_dir}/ROC/{model}_lows_test",
        ...
        if high_test_data is not None:
            ...
            if float(best_params_dict[model]['valid_acc_mean']) >= float(previous_models[model]['valid_acc_mean']):
                ...
                save_confusion_matrix(fig, f"{args.output_dir}/confusion_matrices/{model}_highs_test",
                                      acc=high_test_score)
                try:
                    best_params_dict[model]['test_highs_auc'] = save_roc_curve(m, high_test_data, high_test_classes,
                                                                         unique_labels,
                                                                         f"{args.output_dir}/ROC/{model}_highs_test",
        ...
        if vhigh_test_data is not None:
            ...
            if float(best_params_dict[model]['valid_acc_mean']) >= float(previous_models[model]['valid_acc_mean']):
                ...
                save_confusion_matrix(fig, f"{args.output_dir}/confusion_matrices/{model}_vhighs_test",
                                      acc=vhigh_test_score)
                try:
                    best_params_dict[model]['test_vhighs_auc'] = save_roc_curve(m, vhigh_test_data, vhigh_test_classes,
                                                                         unique_labels,
                                                                         f"{args.output_dir}/ROC/{model}_vhighs_test",
        ...
        if float(best_params_dict[model]['valid_acc_mean']) >= float(previous_models[model]['valid_acc_mean']):
            model_filename = f"{args.output_dir}/saved_models/sklearn/{model}.sav"
            with open(model_filename, 'wb') as file:
                pickle.dump(m, file)
            scaler_filename = f"{args.output_dir}/saved_models/sklearn/scaler_{model}.sav"
            with open(scaler_filename, 'wb') as file:
                pickle.dump(scaler, file)

            fig = get_confusion_matrix(test_classes, y_preds_test, unique_labels)
            save_confusion_matrix(fig, f"{args.output_dir}/confusion_matrices/{model}_test", acc=test_score)
            try:
                save_roc_curve(m, test_data, test_classes, unique_labels, f"{args.output_dir}/ROC/{model}_test",
                               binary=args.binary,
                               acc=test_score)
            except:
                print('No proba function, or something else.')
    ...
    with open(f'{args.output_dir}/saved_models/sklearn/best_params.json', "w") as read_file:
